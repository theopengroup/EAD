...
#include <climits>
#include "Config.h"
//#include "Trace_Readers_MSR.h"
#include "Trace_Readers_Dedup.h"
...
//	long long	traceStartTime[traceNumber];
    ...
    for (int i=0; i<traceNumber; i++)
    {
        ...
        readDedupLine(currentTraceLineVector[i], currentTraceLineStartPageVector[i], currentTraceLineEndPageVector[i], currentReadWriteFlagVector[i]);
...
//    print1DVector(traceStartTimeVector);
    ...
    while (!finish)
    {
        ...
        for (; currentWritePage<=currentTraceLineEndPageVector[i]; currentWritePage++)
        {
            ...
            // NoCache
            NoCache_Cache.input(currentWritePage*10000+i, currentReadWriteFlagVector[i]);
...
//        }// end of monitor
        ...
        if (!getline(*traceFilesVector[i], currentTraceLineVector[i]))
        {
            ...
            currentTraceLineTimeVector[i]=9223372036854775807;
    ...
    cout<<"No Cache: MDr="<<NoCache_Cache.getReadCounter()<<"\tMDw="<<NoCache_Cache.getWriteCounter()<<endl;
